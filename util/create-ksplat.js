...
if (process.argv.length < 4) {
    ...
    console.log('Usage: node create_splat.js [path to .PLY or .SPLAT] [output file name] [compression level = 0] [alpha removal threshold = 1]');
    process.exit(1);
}
...
function fileBufferToSplatBuffer(fileBufferData, isPly, isStandardSplat, compressionLevel, alphaRemovalThreshold) {
    ...
    if (isPly) {
        ...
        splatBuffer = plyParser.parseToSplatBuffer(compressionLevel, alphaRemovalThreshold);
    } else {
        if (isStandardSplat) {
