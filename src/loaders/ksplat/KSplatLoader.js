...
export class KSplatLoader {
   ...
   static checkVersion(buffer) {
    ...
    static loadFromURL(fileName, externalOnProgress, directLoad, onSectionBuilt) {
        ...
        return fetchWithProgress(fileName, localOnProgress, !directLoad).then((fullBuffer) => {
            if (externalOnProgress) externalOnProgress(0, '0%', LoaderStatus.Processing);
            const loadPromise = directLoad ? directLoadPromise.promise : KSplatLoader.loadFromFileData(fullBuffer);
